namespace TargetHound.Services
{
    ...
    public class UserService : IUserService
    {
        ...
        {
            var projectName = this.dbContext.Projects.SingleOrDefault(x => x.Id == projectId && x.IsDeleted == false).Name;
            var projectInvitationMessage = MessageTemplates.ProjectInvitation(senderName, receiverName, projectName, linkToJoin);
            await this.emailSender.SendEmailAsync(senderEmail, senderName, receiverEmail, receiverName, MessageTemplates.InvitationSubject(senderName), projectInvitationMessage);
        }
        ...
        {
            var clientName = this.dbContext.Clients.SingleOrDefault(x => x.Id == clientId && x.IsDeleted == false).Name;
            var clientInvitationMessage = MessageTemplates.ClientInvitation(senderName, clientName, linkToJoin);
            await this.emailSender.SendEmailAsync(senderEmail, senderName, receiverEmail, receiverName, MessageTemplates.InvitationSubject(senderName), clientInvitationMessage);
        }
    }
}
